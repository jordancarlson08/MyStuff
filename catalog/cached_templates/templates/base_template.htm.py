...
_magic_number = 9
_modified_time = 1395967780.036783
_enable_loop = True
...
def render_body(context,**pageargs):
    ...
    try:
        ...
        cart_length = len(cart.keys())
        
        
        __M_locals_builtin_stored = __M_locals_builtin()
        __M_locals.update(__M_dict_builtin([(__M_key, __M_locals_builtin_stored[__M_key]) for __M_key in ['cart_length','cart'] if __M_key in __M_locals_builtin_stored]))
        # SOURCE LINE 61
        __M_writer('\n')
        # SOURCE LINE 62
        if cart_length !=0:
