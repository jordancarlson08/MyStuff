...
_magic_number = 9
_modified_time = 1396751561.089647
_enable_loop = True
    ...
    try:
        ...
        if cart_all:
            ...
            if cart_all.repairItem_list:
                # SOURCE LINE 75
                __M_writer("\t<h3>Repairs</h3>\r\n\t<table class='table table-striped'>\r\n\t\t<thead>\r\n\t\t\t<th>Name</th>\r\n\t\t\t<th>Est. Complete</th>\r\n\t\t\t<th></th>\r\n\t\t</thead>\r\n\t\t<tbody>\r\n\t\t\t<!-- Repair Items -->\r\n")
                # SOURCE LINE 84
                for c in cart_all.repairItem_list:
                    # SOURCE LINE 85
                    __M_writer('\t\t\t<tr>\r\n\t\t\t\t<td>')
                    # SOURCE LINE 86
                    __M_writer(str(c.repair.user.first_name))
                    __M_writer(' ')
                    __M_writer(str(c.repair.user.last_name))
                    __M_writer('</td>\r\n\t\t\t\t<td>')
                    # SOURCE LINE 87
                    __M_writer(str(c.repair.estComplete))
                    __M_writer("</td>\r\n\t\t\t\t<td><a href='/catalog/cart__delete/")
                    # SOURCE LINE 88
                    __M_writer(str(c.repair.id))
                    __M_writer("/repair/'><button class='btn btn-danger btn-sm'><span class='glyphicon glyphicon-remove'></span></button></a></td>\r\n\t\t\t</tr>\r\n")
                # SOURCE LINE 91
                __M_writer('\t\t</tbody>\r\n\t</table>\r\n\t<br/>\r\n')
            # SOURCE LINE 95
