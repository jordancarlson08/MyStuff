...
def process_request(request):
	...
	desc = request.urlparams[1]
	orderby = request.urlparams[0]
	orderby = desc + orderby

	if orderby == '':
		catItems = hmod.CatalogItem.objects.all().order_by('-isActive', 'manufacturer')
	else:
		catItems = hmod.CatalogItem.objects.all().order_by('-isActive', orderby)
